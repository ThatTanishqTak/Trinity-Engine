...
namespace Engine
{
    ...
    public:
        ...
        void RecordAll(VkCommandBuffer command, uint32_t imageIndex, uint32_t currentFrame, VulkanRenderer& renderer);

        IRenderPass* FindPass(const char* passName);
        const IRenderPass* FindPass(const char* passName) const;
