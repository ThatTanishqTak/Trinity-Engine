...
namespace
{
    ...
    VkDebugUtilsMessengerCreateInfoEXT MakeDebugMessengerCreateInfo()
    {
        ...
        l_CreateInfo.messageSeverity = VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT 
...
namespace Engine
{
    ...
    void VulkanRenderer::Initialize(Window* window)
    {
        ...
        m_Swapchain.Initialize(m_PhysicalDevice, m_Device, m_Surface, m_NativeWindow, l_Indices.GraphicsFamily.value(), l_Indices.PresentFamily.value());
        m_Swapchain.Create();

        m_Command.Initialize(m_Device, l_Indices.GraphicsFamily.value());
        m_Command.Create(s_MaxFramesInFlight);

        m_Resources.Initialize(m_PhysicalDevice, m_Device, m_GraphicsQueue, m_Command.GetCommandPool());

        CreateRenderPass();

        m_Descriptors.Initialize(m_Device);
        ...
        m_Sync.Initialize(m_Device, s_MaxFramesInFlight);
    ...
    void VulkanRenderer::BeginFrame()
    {
        ...
        VkResult l_Acquire = vkAcquireNextImageKHR(m_Device, m_Swapchain.GetHandle(), std::numeric_limits<uint64_t>::max(), m_Sync.GetImageAvailableSemaphore(m_CurrentFrame),
    ...
    void VulkanRenderer::EndFrame()
    {
        ...
        Engine::Utilities::VulkanUtilities::VKCheckStrict(vkQueueSubmit(m_GraphicsQueue, 1, &l_SubmitInfo, m_Sync.GetInFlightFence(m_CurrentFrame)), "vkQueueSubmit");
    ...
    uint32_t VulkanRenderer::RateDeviceSuitability(VkPhysicalDevice device) const
    {
    ...
    void VulkanRenderer::CreateLogicalDevice()
    {
        ...
        {
            l_Indices.GraphicsFamily.value(),
    ...
    void VulkanRenderer::CreateFramebuffers()
    {
        ...
        for (size_t i = 0; i < a_ImageViews.size(); ++i)
        {
            ...
            Engine::Utilities::VulkanUtilities::VKCheckStrict(vkCreateFramebuffer(m_Device, &l_FBInfo, nullptr, &m_Framebuffers[i]), "vkCreateFramebuffer");
        }
    }
    ...
    void VulkanRenderer::DestroyDescriptors()
    {
        ...
        m_Descriptors.Shutdown();
        m_Descriptors.Initialize(m_Device); // keep object usable if someone calls recreate paths before shutdown
    ...
    void VulkanRenderer::CleanupSwapchain()
    {
        ...
        {
            vkDestroyFramebuffer(m_Device, it_FB, nullptr);
        }
