...
namespace Trinity
{
    ...
    {
        if (!m_Pipeline || !passContext.ActiveCamera || !passContext.DrawList || !passContext.Stats)
        {
            return;
        }
        ...
        if (!passContext.DrawList->empty())
        {
            ...
            for (const auto& it_DrawCommand : *passContext.DrawList)
            {
                ...
                l_PushBlock.BaseColor[0] = it_DrawCommand.BaseColor.r;
                l_PushBlock.BaseColor[1] = it_DrawCommand.BaseColor.g;
                l_PushBlock.BaseColor[2] = it_DrawCommand.BaseColor.b;
                l_PushBlock.BaseColor[3] = it_DrawCommand.BaseColor.a;
