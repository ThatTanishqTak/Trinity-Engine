...
namespace Trinity
{
    std::optional<ImageData> Image::Load(const std::filesystem::path& path, bool srgb)
    {
        ...
        if (stbi_is_hdr(l_PathString.c_str()))
        {
            ...
            if (l_Data == nullptr)
            {
                TR_CORE_ERROR("Image: failed to load HDR '{}' ({})", l_PathString, stbi_failure_reason());
                return std::nullopt;
            }
        ...
        else
        {
            ...
            if (l_Data == nullptr)
            {
                TR_CORE_ERROR("Image: failed to load '{}' ({})", l_PathString, stbi_failure_reason());
                return std::nullopt;
            }
        ...
        TR_CORE_INFO("Image: loaded '{}' ({}x{}, {})", l_PathString, l_Image.Width, l_Image.Height, l_Image.IsHDR ? "HDR" : "LDR");
